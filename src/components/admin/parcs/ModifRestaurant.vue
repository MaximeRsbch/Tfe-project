...
onMounted(() => {
    parcsStore.fetchRestaurantsById(idRestaurant);
});
...
setTimeout(() => {
    ...
    for (let i = 0; i < resto.value.length; i++) {
        ...
        longitude.value = resto.value[i].longitude;
        ouverture.value = resto.value[i].beginHour;
        fermeture.value = resto.value[i].endHour;
        description.value = resto.value[i].description;
    }
}, 500);

async function updateResto() {
    Swal.fire({
...
<template>
    <div>
        <div v-if="role !== 'user'" class="py-10">
            <section
            ...
            >
                ...
                <form @submit.prevent="updateResto">
                    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                        <div class="hidden">
                            <label class="text-gray-700" for="id"
                                >Id du Restaurant</label
                            >
